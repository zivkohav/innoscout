...
const App: React.FC = () => {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = (event) => {
          ...
          // Auto-fill input if empty to help user
          if (!searchInput) {
  ...
  const handleAction = async () => {
    ...
    if (selectedFile) {
        ...
        try {
            const results = await findStartups(searchInput);
            if (results.length > 0) {
                ...
                <div className="relative">
                   <div className="flex gap-2">
                       <div className="relative flex-1">
                           <input
                                ...
                                className={`w-full bg-slate-900 border ${selectedFile ? 'border-violet-500/50' : 'border-slate-700'} rounded-xl p-4 pl-12 pr-12 text-slate-200 focus:ring-2 focus:ring-violet-500 focus:outline-none transition-all`}
                            />
                            {selectedFile ? (
                   ...
                   {selectedFile && (
                       <div className="absolute top-full left-0 mt-2 flex items-center gap-2 bg-violet-900/20 border border-violet-500/30 text-violet-200 text-xs px-2 py-1 rounded animate-in fade-in slide-in-from-top-1">
                           ...
                           <button onClick={clearFile} className="hover:text-white ml-1"><X className="w-3 h-3" /></button>
                       </div>
                   )}
                </div>
              )}
